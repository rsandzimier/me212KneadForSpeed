...
class LineTrajectory(object):
    ...
    def closest_on_seg(self, v, w, p):
        # Return minimum distance between line segment vw and point p
        l2 = np.sum((w-v)**2)  # i.e. |w-v|^2 -  avoid a sqrt
        if l2 == 0.0:
            return self.distance(p, v)   # v == w case
        # Consider the line extending the segment, parameterized as v + t (w - v).
        # We find projection of point p onto the line. 
        # It falls where t = [(p-v) . (w-v)] / |w-v|^2
        # We clamp t from [0,1] to handle points outside the segment vw.
        t = np.maximum(0, np.minimum(1, np.dot(p - v, w - v) / l2))
        projection = v + t * (w - v)  # Projection falls on the segment
        return self.distance(p, projection), projection

    def closest_point(self, points, p):
        # Returns the closest point in the piecewise linear function
        ...
        for i in xrange(1, len(points)):
            v = points[i-1]
            w = points[i]
            dist, coord = self.closest_on_seg(v, w, p)
            dists.append(dist)
            coords.append(coord)
        return np.argmin(dists), coords[np.argmin(dists)]

    def find_intersection(self, cp1, cp2, pos, r):
        """
        Returns intersections of circle and line segmen
        """
        Q = pos                  # Center of circle
        P1 = cp1      # Start of line segment
        V = cp2 - cp1  # Vector along line segment
        ...
        if 0 <= t2 <= 1:
            p2 = P1 + t2 * V
            if res is not None:
                if self.distance(p2, cp2) < self.distance(res, cp2):
                    res = p2

        return res is not None, res

    def find_drive_point(self, pos, r):
        points = np.array(self.points)
        seg, closest_pt = self.closest_point(points, pos)
        if all(closest_pt - points[-1]) < CLOSE_ENOUGH:
            print 'done'
            return False, np.array([0, 0])
        for i in range(len(points)-2, seg-1, -1):
            cp1 = points[i]
            cp2 = points[i+1]
            inter_found, intersection = self.find_intersection(cp1, cp2, pos, r)
            if inter_found:
                return True, intersection
        return True, closest_pt
